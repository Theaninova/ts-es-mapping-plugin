package com.github.wulkanat.stappsesmapping.annotator

import com.github.wulkanat.stappsesmapping.ESArguments
import com.github.wulkanat.stappsesmapping.ESTypeTags
import com.github.wulkanat.stappsesmapping.ElasticsearchHighlighter
...
class ESTypesAnnotator : Annotator {
    ...
    private fun visitTypeScriptPropertySignature(element: TypeScriptPropertySignature, holder: AnnotationHolder) {
        val tags = element.children.filterIsInstance<PsiComment>().firstOrNull()?.children?.filterIsInstance<JSDocTag>()
                ?: return
    ...
    private fun visitDocTag(element: JSDocTag, holder: AnnotationHolder) {
        when (element.name) {
            ...
            "aggregatable" -> {
                ...
                val tag = ESTypeTags.values().firstOrNull { it.tag.tagString == element.name } ?: return
                val annotation = holder.createAnnotation(TypeScriptAnnotatorCheckerProvider.TS_INFORMATION, element.textRange, "")
                annotation.textAttributes = ElasticsearchHighlighter.ES_KEYWORD
